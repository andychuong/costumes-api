...
function getOneCostume (req, res, next) {
  ...
  res.status(200).json({ data: data.costume })
}
...
function deleteCostume (req, res, next) {
  ...
  res.status(204).json({ data: data.costume})
}

module.exports = {createCostume, getAll, getOneCostume, updateCostume, deleteCostume}
